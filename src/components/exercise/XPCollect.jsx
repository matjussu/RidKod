...
const XPCollect = ({ totalXP, onContinue }) => {
  ...
  // Gérer le tap
  const handleTap = () => {
    if (isAnimating || tapCount >= 9) return;
    ...
    typeWord(currentWord, () => {
      ...
      // Si c'était le dernier tap (index 8 = 9ème tap)
      if (currentTapIndex === 8) {
        handleFinalTap();
      }
    });
  };
  ...
  const handleFinalTap = useCallback(() => {
    setTimeout(() => {
      ...
      // Transition après 1.5s
      setTimeout(() => {
        onContinue();
      }, 1500);
    }, 400);
  }, [triggerSuccess, onContinue]);
  ...
  return (
    <div className="xp-collect-container" onClick={handleTap}>
      ...
      {/* Terminal Block */}
      <div className={`terminal-block ${tapCount === 9 ? 'success' : ''}`}>
      ...
      <div className="tap-instructions">
        <p className="tap-instruction">
          {tapCount < 9 ? 'TAP TO COMMIT' : 'SUCCESS!'}
        </p>
        <p className="tap-counter">
          {tapCount < 9 ? `[${tapCount}/9]` : '[9/9]'}
        </p>
      </div>
